...
d2 <- data.frame(quant_fit2$coefficients)
d2$Variable <- rownames(d2)
d2 <- d2[which(d2$Variable %in% c("age", "high_BP", "diabetesYes", "cancerYes", "health_rateFair" , "health_rateGood", "health_ratePoor" , "health_rateVery Good",
                                  "own_rent_homeOwn", "follow_stockmarketSomewhat Closely" , "follow_stockmarketVery Closely")),]
rownames(d2) <- NULL
colnames(d2)[1] <- "Coef. Est 50%"
d2$"Signif. 50%" <- c(rep("Y", 10))

d3 <- data.frame(quant_fit3$coefficients)
d3$Variable <- rownames(d3)
d3 <- d3[which(d3$Variable %in% c("age", "high_BP", "diabetesYes", "cancerYes","health_rateFair" , "health_rateGood", "health_ratePoor" , "health_rateVery Good",
                                  "own_rent_homeOwn", "follow_stockmarketSomewhat Closely" , "follow_stockmarketVery Closely")),]
rownames(d3) <- NULL
colnames(d3)[1] <- "Coef. Est 90%"
d3$"Signif. 90%" <- c("N","Y","Y","Y","Y","N","N","Y","Y","Y")


d_all <- cbind(d2, d3)
d_all <- d_all[,c("Variable", "Coef. Est 50%", "Signif. 50%","Coef. Est 90%","Signif. 90%")]
#saveRDS(d_all, file = "/home/guest/sem_3/707/health_retirement/scripts/R/deliverable/coef_quantile_reg.rds")
